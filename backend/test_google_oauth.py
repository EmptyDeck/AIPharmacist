...
class GoogleOAuthTester:
    ...
    def test_4_token_management(self):
        ...
        # 토큰 파일 확인 (UserTokenManager와 동일한 로직 사용)
        import os
        safe_user_id = "".join(c for c in self.user_id if c.isalnum() or c in ('-', '_'))
        token_file = f"user_tokens/google_token_{safe_user_id}.json"
