        ...
        if category_dir.is_dir():
            for file_path in category_dir.iterdir():
                if file_path.is_file():
                    ...
                    vision_result = get_vision_result(file_id)
                    if vision_result:
                        file_info["has_vision_result"] = vision_result.get("success", False)
...
async def process_vision(file_id: str):
    ...
    if file_id in vision_retry_tracker:
        ...
        if retry_info['count'] >= VISION_RETRY_LIMIT:
            ...
            if time_since_last < VISION_RETRY_COOLDOWN:
                ...
                # 쿨다운 후 리셋
                vision_retry_tracker[file_id] = {'count': 0, 'last_attempt': current_time}
    else:
        # 첫 시도
        vision_retry_tracker[file_id] = {'count': 0, 'last_attempt': current_time}
    ...
    try:
        ...
        remaining_attempts = VISION_RETRY_LIMIT - vision_retry_tracker[file_id]['count']
