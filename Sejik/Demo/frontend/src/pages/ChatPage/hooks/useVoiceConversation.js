...
export function useVoiceConversation({
  ...
  onBotMessage,
  autoStart = false,
  silenceThreshold = 0.0, // 침묵 임계값 (0-1)
  silenceDuration = 7000, // 침묵 지속 시간 (ms) Xms동안 침묵이 감지되면 녹음이 자동으로 중지됩니다.
  ...
  const analyzeAudio = useCallback(() => {
    ...
    // 🆕 실시간 오디오 레벨 모니터링
    const volumeBar = "█".repeat(Math.floor(normalizedVolume * 20));
    const thresholdBar = "█".repeat(Math.floor(silenceThreshold * 20));
    const status = normalizedVolume > silenceThreshold ? "🔊 SOUND" : "🔇 SILENCE";
    
    console.log(
      `[📊] 현재: ${normalizedVolume.toFixed(3)} ${volumeBar.padEnd(20, "░")} | ` +
      `임계값: ${silenceThreshold.toFixed(3)} ${thresholdBar.padEnd(20, "░")} | ${status}`
    );
    ...
    // 침묵 상태이고 타이머가 없다면 타이머 시작
      ...
      if (silenceTime >= silenceDuration) {
        ...
        console.log(`[⏱️] 침묵 타이머 시작 (${remainingTime}ms 남음, 현재 침묵: ${silenceTime}ms)`);
