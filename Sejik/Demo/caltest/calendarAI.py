...
from utils.googleCalender.cal_agent import calendar_agent   # 싱글톤 인스턴스
...
답변:"""
    ...
    def process_calendar_addition(self, user_id: str, original_medication_text: str) -> dict:
        try:
            events = text_to_cal_converter.process_medication_text(original_medication_text)
            if not events:
                return {"success": False, "message": "복약 정보를 캘린더 형식으로 변환할 수 없습니다."}

            result = calendar_agent.add_medication_schedule(user_id, events)
            return result | {"added_count": result.get("events_added", 0)}   # 파이썬3.9+
        except Exception as e:
            return {"success": False, "message": f"캘린더 추가 중 오류: {e}", "error": str(e)}
