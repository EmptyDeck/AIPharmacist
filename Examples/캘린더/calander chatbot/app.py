...
class WebMedicationSchedulerAgent:
    ...
    def process_patient_text_web(self, patient_text, duration_days=30):
        ...
        for i, med in enumerate(medications, 1):
            result['messages'].append(f"{i}. {med['name']} - 하루 {med['frequency']}회")
            result['messages'].append(f"   복용시간: {', '.join(med['times'])}")
        
        # 2. 일정 생성
        schedule_result = self.create_medication_schedule(medications, duration_days)
        ...
        result['messages'].append(f"🎉 총 {len(schedule_result['created_events'])}개의 약물 복용 일정이 생성되었습니다!")
...
def process_medication():
    try:
        ...
        result = medication_agent.process_patient_text_web(patient_text, duration_days)
